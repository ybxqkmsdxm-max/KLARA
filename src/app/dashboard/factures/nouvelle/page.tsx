...
export default function NouvelleFacturePage() {
  ...
  const [terms, setTerms] = useState(
    "Paiement à réception de la facture. Tout retard de paiement entraînera des pénalités de retard."
  );
  ...
  const handleSubmit = async (sendImmediately: boolean) => {
    ...
    if (!dueDate) {
      toast.error("Veuillez indiquer la date d'échéance");
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/factures", {
        ...
        body: JSON.stringify({
          ...
          status: sendImmediately ? "ENVOYEE" : "BROUILLON",
        }),
      });
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {dueDatePresets.map((preset) => (
                  <Button
                    ...
                    onClick={() => handleDueDatePreset(preset.days)}
                  >
                    {preset.label}
                  </Button>
                ))}
